...
NSample = NSTFT
Spgr = cp.zeros((NSTFT,int(Nfft/2+1)), dtype='complex128')
MM = cp.zeros((int(Nfft/2+1)), dtype='complex128')
fid = open(file,'rb')
jj = 0
...
for ii in range(NSTFT) : 
    Ofst = 0 if not ii else -(NOvlp*4*NbVoies)
    data = cp.fromfile(fid, count = NTrt*NbVoies, offset = Ofst, dtype='int32')    
    Mems = cp.reshape(data,(-1, NbVoies)).T[128,:] #V
    Spgr[ii,:] = cp.fft.rfft(Mems*fen, Nfft)    
    Sig.append(Mems[NOvlp:].T)
    Lp = cp.sqrt(cp.std(cp.abs(Spgr[ii,:]))/ ((ii+1)*NTrt) ).get() 
...
Sog = cp.array(Sig).get().reshape(-1,1)
freq = np.fft.rfftfreq(Nfft,1/Fe)
fig = make_subplots(rows=2, cols=1)
fig.add_trace(go.Scatter(x=t,y=Sog.ravel()), row=1, col=1)
fig.add_trace(go.Heatmap(x=tt,y=freq,z=20*np.log10(np.abs(Spgr.T)).get(),zmin = 80, zmax = 120, colorscale='inferno'), row=2, col=1)
...
SpgZ = Spgr.get()[np.logical_and(tt >= tmin, tt<tmax),:]
...
# R = cp.array(cdist(V,Xm[:256,:]))
# omg = 2*np.pi*cp.array(Frq)
# for o in omg:
#     G = cp.exp(-1j*o*R/C)
# S = cp.einsum('fsm,mft->st', G, P)
